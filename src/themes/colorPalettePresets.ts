...
export const defaultPalette: DetailedPalette = {
  light: {
    ...
    border: {
     ...
     syntax: {
      // === Типы и структуры ===
      type: 'cerulean',        // class, interface, type
      enum: 'pinks',           // enum
      interface: 'lavender',   // interface
      class: 'pinks',          // class
      struct: 'cerulean',      // struct (C++, Rust)

      // === Переменные и параметры ===
      variable: 'mints',           // обычные переменные
      parameter: 'greens',         // параметры функций
      property: 'mints',           // свойства объектов
      field: 'greens',             // поля классов
      constant: 'mints',           // константы (const, readonly)
      local: 'slateInk',           // локальные переменные

      // === Функции и методы ===
      function: 'blues',           // обычные функции — отличный от type
      method: 'blues',             // методы
