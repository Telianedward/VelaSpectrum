...
export default class NumberAdjuster {
     ...
     */
    public static adjustWithPrecision = (
        type: string,
        value: any,
        precision: number = 0
    ): number => {
        ...
        const adjustedValue: number = type === 'round'
    ...
    public static trig(
        ...
        if (this.abs(result) < 1e-10) result = 0;
     ...
     */
    public static getAngleWithPrecision (
        y: number,
        x: number,
        precision: number | null = null,
     ...
     */
    public static rootWithPrecision = (
        value: number,
        rootDegree: number = 3,
        precision: number | null = null
    ): number => {
        ...
        // Чётная степень корня из отрицательного — не определена в вещественных числах
        if ( isNegative && rootDegree % 2 === 0 ) {
            return NaN;
        }

        // Корень n-й степени: x^(1/n), а не x^n
        const result = isNegative
            ? -Math.pow( absValue, 1 / rootDegree )
            : Math.pow( absValue, 1 / rootDegree );
    ...
    public static extremum(
        ...
        const filtered:number[] = numbers.filter((n : number) : boolean => !isNaN(+n));
        if (filtered.length === 0) return NaN;

        const result: number = type === 'min'
            ? Math.min(...filtered)
            : Math.max(...filtered);


        if ( precision != null ) {
            const precisionValue: number = precision ?? 0;
    ...
    public static pow(
        ...
        const result: number = Math.pow(numBase, numExponent);
                // Применяем коррекцию с заданной точностью
        if ( precision != null ) {
            return this.adjustWithPrecision( 'round', result, precision );
        }
     ...
     * @returns
     */
    public static adjustWithPrecision3 = (
        type: string,
        value: any,
        precision: number = 0
    ): number => {
        ...
        value = (
            ( type == `round` ) ? (
                Math.round( +( value[ 0 ] + `e` + ( value[ 1 ] ? ( +value[ 1 ] - precision ) : -precision ) ) )
            ) : (
                ( type == `floor` ) ? (
                    Math.floor( +( value[ 0 ] + `e` + ( value[ 1 ] ? ( +value[ 1 ] - precision ) : -precision ) ) )
                ) : ( ( type == `ceil` ) ? (
                    Math.ceil( +( value[ 0 ] + `e` + ( value[ 1 ] ? ( +value[ 1 ] - precision ) : -precision ) ) )
                ) : Math.round( +( value[ 0 ] + `e` + ( value[ 1 ] ? ( +value[ 1 ] - precision ) : -precision ) ) )
                )
            ) );
        value = value.toString().split( `e` );
