 ...
 */
  /**
   * Линейный RGB → sRGB с защитой от отрицательных каналов (иначе Math.pow даёт NaN).
   */
  private static linearChannelToSrgb ( channel: number ): number {
  ...
  public static oklchToRgb ( oklch: { l: number; c: number; h: number; } ): { r: number; g: number; b: number; } {
    ...
    // Сжимаем хрому, пока linear RGB не войдёт в sRGB (предпочтительно вместо жёсткого clamp каналов)
    let linearRgb = this.oklabToLinearRgb( this.oklchToOklab( { l, c, h } ) );
    let guard = 0;
    while ( !this.isInSrgb( linearRgb ) && c > 1e-4 && guard < 48 ) {
      c *= 0.92;
      linearRgb = this.oklabToLinearRgb( this.oklchToOklab( { l, c, h } ) );
      guard++;
    }

    // Финальный clamp ≥ 0 перед гаммой (на случай остаточного out-of-gamut)
    const sRgb = {
      r: this.linearChannelToSrgb( linearRgb.r ),
      g: this.linearChannelToSrgb( linearRgb.g ),
      b: this.linearChannelToSrgb( linearRgb.b ),
    };
